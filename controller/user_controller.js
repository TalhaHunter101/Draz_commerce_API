...
const admin = require("../Schema/admin_user");
const Evalid = require("../middlewares/Emailvalidator");
...
exports.signup = async function (req, res) {
  if (req.body.fullname == "" || req.body.fullname == undefined) {
    ...
    await seller.findOne({ email: req.body.email }).then((user) => {
      if (user) {
        // user already present
        return res
          .status(409)
    ...
    res.status(400).json({
      Message: "Type should be 'admin', 'coustomer' or 'seller' (lowercase)",
    });
    return;
  }
};
